...
use tytanic_core::test_set::{eval, TestSet};
...
pub fn run(ctx: &mut Context, args: &Args) -> eyre::Result<()> {
    ...
    for old in suite.nested().keys() {
        ...
        let collision = suite.matched().contains_key(&new) || suite.filtered().contains_key(&new);
...
fn migrate_test(paths: &Paths, old: &Id, new: &Id) -> eyre::Result<()> {
    ...
    stdx::fs::remove_dir(&out_dir, true)?;
    let diff_dir = paths.test_diff_dir(old);
    stdx::fs::remove_dir(&diff_dir, true)?;
    Ok(())
}
