...
use crate::test_set::ExpressionFilterState;
...
impl Filter for CombinedFilter {
    type State<'a>
        = CombinedFilterState<'a>
    where
        Self: 'a;

    fn state(&self) -> Self::State<'_> {
        CombinedFilterState {
            test_set: self.test_set.as_ref().map(Filter::state),
            exact: self.exact.as_ref().map(Filter::state),
        }
    }
}

/// Created by [`Filter::state`] for [`CombinedFilter`].
#[derive(Debug, Default)]
pub struct CombinedFilterState<'f> {
    test_set: Option<ExpressionFilterState<'f>>,
    exact: Option<ExactFilterState>,
}
...
/// Returned by [`FilterState::filter`] or [`FilterState::finish`] for [`CombinedFilterState`].
#[derive(Debug, Error)]
pub enum Error {
