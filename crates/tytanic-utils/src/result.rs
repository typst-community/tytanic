...
pub trait ResultEx<T, E>: Sealed {
    ...
    /// assert_eq!(
    ///     fs::read_to_string("foo.txt").attach_path("foo.txt")?,
    ///     String::from("foo"),
    /// );
    /// # Ok::<_, Box<dyn std::error::Error>>(())
    /// ```
    fn attach_path<P>(self, path: P) -> Result<T, PathError<E>>
    where
        P: Into<PathBuf>;
    ...
    /// // if foo doesn't exist we get an error with the path attached to it
    /// assert_eq!(
    ///     fs::read_to_string("foo.txt").attach_path_with(|| "foo.txt".into())?,
    ///     String::from("foo"),
    /// );
    /// # Ok::<_, Box<dyn std::error::Error>>(())
    /// ```
    fn attach_path_with<F>(self, f: F) -> Result<T, PathError<E>>
    where
        F: FnOnce() -> PathBuf;
}
