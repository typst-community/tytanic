...
#import "/book.typ": book-page, heading-reference
...
Once installed you will have a `tt` binary available, make sure to have a look at #shiroa.cross-link("/src/quickstart/install.typ", reference: <Dependencies>)[Dependencies] if running Tytanic spits out some error about dynamic libraries.

= Methods
== Download from GitHub
You can download pre-built binaries of all stable versions from the #link("https://github.com/typst-community/tytanic/releases/")[release page] of the GitHub repository, these are automatically built for Linux, macOS, and Windows.
...
To install Tytanic from source, you must have a Rust toolchain (Rust *v1.84.0+*) and `cargo` installed, you can get these using #link("https://www.rust-lang.org/tools/install")[`rustup`].
...
Check out the #link("https://github.com/users/typst-community/packages/container/tytanic")[package] for platform specific builds.
...
== OpenSSL
OpenSSL (*v1.0.1* to *v3.x.x*) or LibreSSL (*v2.5* to *v3.7.x*) are required to allow Tytanic to download packages from the #link("https://typst.app/universe")[Typst Universe] package registry.
