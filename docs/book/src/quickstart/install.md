...
To install Tytanic from source, you must have a Rust toolchain (Rust **v1.96.0+**) and `cargo` installed, you can get these using [`rustup`][rustup].

#### Stable
```shell
cargo install --locked tytanic@0.3.4
```
...
#### Stable
```shell
nix run github:typst-community/tytanic/v0.3.4
```
...
```shell
docker pull ghcr.io/typst-community/tytanic:v0.3.4
```
