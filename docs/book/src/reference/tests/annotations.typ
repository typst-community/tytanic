...
#show: book-page.with(title: "Annotations")
...
#table(
  columns: 2,
  table.header[Annotation][Description],
  table.hline(),

  [`skip`], [Marks the test as part of the `skip()` test set.],

  [`dir`],
  [Sets the direction used for creating difference documents, expects either `ltr` or `rtl` as an argument.],

  [`ppi`],
  [Sets the pixel per inch used for exporting and comparing documents, expects a floating point value as an argument.],

  [`max-delta`],
  [Sets the maximum allowed per-pixel delta, expects an integer between 0 and 255 as an argument.],
